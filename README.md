...
Initially, our code for xmm and ymm does not work in release mode, but it works properly for release after using push and pop for the non-volatile registers used, such as rbx, rsi, and rdi. However, there are problems we encountered when using release mode for ymm are the following: multiplier 1e3 not working for the runtime of each run, total runtime accumulator not adding up the values of the times from each run, and total accumulated time cannot be divided by the number of runs used in the loop. We initially noticed that the equation does not accept double as a multiplier, so we converted it to int. For the total runtime accumulator, it only replaces the value of the total by the current runtime, so it does not accumulate properly; to address this issue, we made an array that will store the runtime of each run and added all of its content to a new variable, and it worked. Lastly, for the division, since it does not accept values equal to 30, we first divided the total by 3 and divided by 10 after. This solution initially solved the problem we encountered on the c implementation side. However, we realized that the problem lies in the ASM file, specifically the non-volatile registers. We thought you didn't need to push and pop the non-volatile ymm and xmm registers since they work fine in debug mode, and they also work in our SIMD AVX2 with XMM implementation in release mode. By pushing and popping registers xmm6 and ymm6-ymm13, we have solved the weird issue we encountered in our code for release mode.
